...
WebManager::WebManager()
    : _server(80), _ws("/ws"), _rtc(nullptr), _config(nullptr),
      _printer(nullptr), _display(nullptr), _history(nullptr),
      _lastWeight(0.0f) {}

void WebManager::begin(TimeManager *rtc, ConfigManager *config,
                       PrinterManager *printer, DisplayManager *display,
...
void WebManager::broadcastCommandData(float gross, float tare, float netto, int state) {
  if (_ws.count() > 0) {
    JsonDocument doc;
...
// Feature Flags for Adaptive UI
#ifdef SUPPORT_USB
    doc["supportUsb"] = true;
#else
            doc["supportUsb"] = false;
#endif

#ifdef MODE_COMMAND
    doc["isCommandMode"] = true;
#else
            doc["isCommandMode"] = false;
#endif
  ...
  _server.on(
      ...
      [this](AsyncWebServerRequest *request, uint8_t *data, size_t len,
        ...
        if (!error) {
          if (doc["companyName"].is<const char *>())
            _config->setCompanyName(doc["companyName"]);
          ...
          if (doc["useStaticIp"].is<bool>()) {
            _config->setStaticIp(doc["useStaticIp"],
          ...
          // 1. Live Update Ukuran Kertas (Tanpa Restart)
          if (doc["paperSize"].is<int>()) {
              int newSize = doc["paperSize"].as<int>();
              _config->setPaperSize(newSize);
              if (_printer) _printer->setPaperSize(newSize);
          }

          // 2. Cek apakah ada perubahan kritikal yang butuh restart
          bool printerTypeChanged = false;
          if (doc["printerType"].is<int>()) {
              if (doc["printerType"].as<int>() != _config->getConfig().printerType) {
                  printerTypeChanged = true;
          ...
          if (doc["wifiSsid"].is<const char*>()) {
              if (doc["wifiSsid"].as<String>() != _config->getConfig().wifiSsid) {
                  wifiChanged = true;
                  _config->setWiFi(doc["wifiSsid"].as<String>(), doc["wifiPassword"] | _config->getConfig().wifiPassword);
              }
          }
          ...
          if (_config->save()) {
            ...
            // Restart HANYA jika nilainya benar-benar berubah (WiFi atau Koneksi Printer)
            if (wifiChanged || printerTypeChanged) {
              _shouldRestart = true;
              _restartTimer = millis();
              Serial.println("WebManager: Critical change detected. Restarting in 2s...");
            }
          } else {
  ...
  _server.on(
      "/api/history", HTTP_DELETE, [this](AsyncWebServerRequest *request) {
  ...
  _server.onNotFound([](AsyncWebServerRequest *request) {
    ...
    // Hanya redirect jika host bukan IP ESP32 (artinya ini request captive
    // portal seperti gstatic.com)
    if (host != apIP.toString() && host != "timbangan.local") {
      request->redirect(String("http://") + apIP.toString() + "/");
    } else {
      // Jika request adalah file CSS/JS yang hilang, kembalikan 404 agar tidak
      // redirect loop
      request->send(404, "text/plain",
                    "File Not Found (Upload File System Image!)");
    }
  });
}
...
void WebManager::onOTAProgress(size_t current, size_t final) {
  ...
  if (millis() - lastUpdate > 500) {
    ...
    if (_display) {
      _display->showMessage("OTA UPDATE",
